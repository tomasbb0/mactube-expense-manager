  ...
  try {
    ...
    switch (action) {
      ...
      case "getByArtist":
        result = {
          success: true,
          expenses: getExpensesByArtist(e.parameter.artist),
        };
        break;
      case "getArtists":
  ...
  try {
    ...
    switch (action) {
      ...
      case "syncToWebsite":
        ...
        break;
      case "backupAndSync":
        const backupExpenses = data.expenses || [];
        Logger.log("Backup & Sync: " + backupExpenses.length + " expenses");
        result = backupAndSync(backupExpenses);
        break;
      case "syncUserData":
        result = syncUserData(
          data.users || [],
...
// ============================================
// BACKUP & SYNC - Creates timestamped backup before overwriting
// ============================================
function backupAndSync(expenses) {
  Logger.log("=== BACKUP & SYNC STARTED ===");

  if (!expenses || !Array.isArray(expenses)) {
    return { success: false, error: "Invalid expenses data" };
  }

  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_IDS.main);
    const despesasSheet = ss.getSheetByName("Despesas");
    let backupName = "";

    // Step 1: Create backup of current Despesas sheet (if it exists and has data)
    if (despesasSheet && despesasSheet.getLastRow() > 1) {
      const now = new Date();
      ...
      // Copy current sheet as backup
      const backupSheet = despesasSheet.copyTo(ss);
    ...
    // Step 2: Now sync the new data (overwrites Despesas)
    const syncResult = syncFromWebsite(expenses);
    ...
    return {
      ...
      message: backupName
        ...
        : "Sem dados anteriores para backup",
    };
  } catch (error) {
    Logger.log("ERROR in backupAndSync: " + error.toString());
    Logger.log("Stack: " + error.stack);
    return { success: false, error: error.toString() };
  }
}
  ...
  Object.keys(byArtist).forEach((artist) => {
    ...
    try {
      ...
      Object.keys(byProject).forEach((project) => {
        ...
        const proveitos = projectExpenses.filter(
          (e) => e.investor !== "maktub",
        );
        ...
        sheet
          ...
          .setValue("Resultado " + artist + " - " + project);
        sheet
        ...
        sheet
          .getRange(currentRow, 1, 1, 4)
          .setValues([["Data", "Tipo", "Nome", "Valor"]]);
        sheet
          .getRange(currentRow, 1, 1, 4)
          .setFontWeight("bold")
          ...
          sheet
            ...
            .setValues([
              [
                ...
                "",
                "*** TOTAL CUSTOS ***",
                "=SUM(D" + custosDataStartRow + ":D" + custosDataEndRow + ")",
              ],
            ]);
        } else {
          sheet
            .getRange(currentRow, 1, 1, 4)
            .setValues([["", "", "*** TOTAL CUSTOS ***", 0]]);
        }
        sheet
          .getRange(currentRow, 1, 1, 4)
          .setFontWeight("bold")
        ...
        sheet
          .getRange(currentRow, 1, 1, 4)
          .setValues([
            ["", "", "RESULTADO", "=D" + proveitosRow + "-D" + custosRow],
          ]);
        sheet
          .getRange(currentRow, 1, 1, 4)
          .setFontWeight("bold")
      ...
      createArtistQGSheet(
        ss,
        artist,
        allProjectSummaries.filter((s) => s.artist === artist),
      );

      Logger.log(
        "Updated " + Object.keys(byProject).length + " projects for " + artist,
      );
    } catch (e) {
...
function createArtistQGSheet(ss, artist, summaries) {
  ...
  // Format currency
  sheet
    .getRange(dataStartRow, 2, currentRow - dataStartRow + 1, 5)
    .setNumberFormat("#,##0.00 €");

  // Add conditional formatting for resultado columns (D and F)
  const resultadoRange = sheet.getRange(
    dataStartRow,
    4,
    currentRow - dataStartRow + 1,
    1,
  );
  const balancoRange = sheet.getRange(
    dataStartRow,
    6,
    currentRow - dataStartRow + 1,
    1,
...
function createMainQGSheet(allSummaries) {
  try {
    ...
    // Format currency
    sheet
      .getRange(dataStartRow, 3, currentRow - dataStartRow + 1, 5)
      .setNumberFormat("#,##0.00 €");
